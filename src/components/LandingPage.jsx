...
// ═══════════════════════════════════════════
// ANIMATED DEMO COMPONENTS (INLINE)
// ═══════════════════════════════════════════
...
const DemoShell = ({ color, steps, moduleName }) => {
  ...
  const play = () => { setActiveStep(0); setIsPlaying(true); setHasStarted(true); };
  const restart = () => { clearInterval(timerRef.current); setActiveStep(0); setIsPlaying(false); setHasStarted(false); };

  return (
    <div style={{ ...glassCard({ padding: 4, overflow: "hidden" }), marginBottom: 48 }}>
      <DemoKeyframes />
      <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", background: `linear-gradient(145deg, ${G.bg}, ${color}06)`, minHeight: 420 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "16px 20px 12px", borderBottom: `1px solid ${G.glassBorder}` }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: isPlaying ? color : G.textDim, boxShadow: isPlaying ? `0 0 8px ${color}60` : "none", transition: "all 0.4s ease" }} />
            <span style={font(11, 500, G.textMuted)}>{moduleName} &middot; Interactive Demo</span>
          </div>
          <div style={{ display: "flex", gap: 6 }}>
            {steps.map((_, i) => (
              <div key={i} style={{ width: i === activeStep ? 20 : 6, height: 6, borderRadius: 3, background: i <= activeStep && hasStarted ? color : G.glassBorder, transition: "all 0.4s ease" }} />
            ))}
          </div>
        </div>
        <div style={{ padding: "28px 24px 24px", minHeight: 340 }}>
          {!hasStarted ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 300, gap: 20 }}>
              <div onClick={play} style={{ width: 80, height: 80, borderRadius: "50%", background: `${color}15`, border: `2px solid ${color}40`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", transition: "all 0.3s ease" }}
                onMouseEnter={(e) => { e.currentTarget.style.background = `${color}25`; e.currentTarget.style.transform = "scale(1.08)"; }}
              ...
              <span style={font(13, 400, G.textMuted)}>See how {moduleName} works</span>
            </div>
          ) : (
            <div>
              {steps.map((step, i) => (
                <div key={i} style={{ opacity: i === activeStep ? 1 : 0, transform: i === activeStep ? "translateY(0)" : "translateY(12px)", transition: "all 0.6s cubic-bezier(0.16, 1, 0.3, 1)", position: i === activeStep ? "relative" : "absolute", pointerEvents: i === activeStep ? "auto" : "none" }}>
        ...
        {hasStarted && (
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 20px 16px", borderTop: `1px solid ${G.glassBorder}` }}>
            <span style={font(11, 400, G.textDim)}>Step {activeStep + 1} of {steps.length}</span>
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={restart} style={{ ...font(11, 500, G.textMuted), background: "none", border: `1px solid ${G.glassBorder}`, borderRadius: 100, padding: "6px 14px", cursor: "pointer" }}>Restart</button>
              {!isPlaying && activeStep < steps.length - 1 && (
                <button onClick={() => setIsPlaying(true)} style={{ ...font(11, 500, "#000"), background: color, border: "none", borderRadius: 100, padding: "6px 14px", cursor: "pointer" }}>Continue</button>
              )}
            </div>
          </div>
        )}
...
const MockCard = ({ children, color, glow, style = {} }) => (
  <div style={{ ...glassCard({ padding: "16px 18px", background: glow ? `${color}08` : G.glass, border: `1px solid ${glow ? color + "25" : G.glassBorder}` }), ...style }}>{children}</div>
);

const AnimBar = ({ width, color, delay = 0 }) => (
  <div style={{ height: 6, borderRadius: 3, background: `${color}30`, overflow: "hidden", marginBottom: 6 }}>
    <div style={{ width, height: "100%", borderRadius: 3, background: color, animation: `barGrow 1.2s ${delay}s ease-out forwards`, transformOrigin: "left" }} />
  </div>
);

const Tag = ({ text, color }) => (
  <span style={{ ...font(10, 600, color), padding: "3px 10px", borderRadius: 100, background: `${color}12`, border: `1px solid ${color}20`, letterSpacing: "0.05em", textTransform: "uppercase" }}>{text}</span>
);
    ...
    render: (c) => (
      <div>
        ...
        <MockCard color={c} style={{ marginBottom: 12 }}>
          ...
          <div style={{ height: 4, borderRadius: 2, background: `${c}15`, overflow: "hidden" }}>
            <div style={{ width: "100%", height: "100%", borderRadius: 2, background: `linear-gradient(90deg, ${c}40, ${c})`, animation: "progressSlide 2s ease-in-out infinite" }} />
          </div>
        </MockCard>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {["CBC", "Metabolic Panel", "Lipids", "Thyroid"].map(t => (
            <span key={t} style={{ ...font(11, 400, G.textMuted), padding: "6px 12px", borderRadius: 8, background: G.glass, border: `1px solid ${G.glassBorder}` }}>{t}</span>
          ))}
        </div>
      </div>
    ),
      ...
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            ...
            { label: "Iron", value: "85 \u00b5g/dL", status: "Normal", statusColor: c, bar: "78%" },
    ...
    render: (c) => (
      <div>
        ...
        <MockCard color={c} glow style={{ marginBottom: 12 }}>
          ...
          <p style={font(13, 400, G.textMuted)}>Based on your labs, I recommend prioritizing Vitamin D supplementation (2000 IU daily) and adjusting your diet to reduce LDL cholesterol. Here is your personalized plan:</p>
        </MockCard>
  ...
  },
  {
    render: (c) => (
      <div>
        ...
        <MockCard color={c} glow style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", gap: 4, alignItems: "flex-end", height: 60 }}>
            {[40, 52, 48, 55, 60, 58, 65, 68, 72, 70, 75, 78].map((v, i) => (
              <div key={i} style={{ flex: 1, height: `${v}%`, background: i === 11 ? c : `${c}30`, borderRadius: "3px 3px 0 0", animation: `barGrow 0.6s ${i * 0.08}s ease-out forwards` }} />
            ))}
          </div>
        ...
        <div style={{ ...font(12, 400, G.textDim), fontStyle: "italic", textAlign: "center", padding: "8px 0" }}>
          Aram detects rising stress &rarr; notifies Porul to lighten your workload
        </div>
      </div>
    ),
  },
          ...
          {[
            ...
            <MockCard key={acc.name} color={c}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <span style={{ fontSize: 18 }} dangerouslySetInnerHTML={{ __html: acc.icon }} />
    ...
    render: (c) => (
      <div>
        ...
        <MockCard color={c} glow style={{ marginBottom: 12 }}>
          ...
          {[
            { cat: "Housing", amount: "$1,800", w: "42%", color: c },
            { cat: "Food & Dining", amount: "$620", w: "14%", color: "#F97316" },
            { cat: "Transportation", amount: "$340", w: "8%", color: "#06B6D4" },
            { cat: "Subscriptions", amount: "$285", w: "7%", color: G.inbam },
            { cat: "Savings & Invest", amount: "$1,235", w: "29%", color: G.aram },
          ].map((cat, i) => (
  ...
  },
  {
    render: (c) => (
      <div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[
            { icon: "\uD83D\uDD14", title: "Subscription alert", msg: "You are paying for 3 overlapping streaming services. Consolidating could save $28/mo.", priority: "Medium" },
            ...
            { icon: "\uD83E\uDDFE", title: "Tax optimization", msg: "You have $2,400 in deductible home office expenses this quarter.", priority: "Action" },
          ].map(insight => (
    ...
    render: (c) => (
      <div>
        ...
        <MockCard color={c} glow style={{ marginBottom: 16 }}>
          ...
          <p style={{ ...font(12, 400, G.textMuted), marginTop: 6 }}>Auto-generates invoices from tracked hours, sends reminders on Day 15, and logs payments when received.</p>
        </MockCard>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {[
            ...
            <div key={s.step} style={{
              ...
              <span style={font(12, s.status === "active" ? 500 : 400, s.status === "pending" ? G.textDim : G.textMuted)}>{s.step}</span>
            </div>
          ))}
        </div>
        <div style={{ ...font(12, 400, G.textDim), fontStyle: "italic", textAlign: "center", padding: "12px 0 0" }}>
          Porul detects cash surplus &rarr; signals Inbam to unlock creative tools budget
        </div>
      </div>
    ),
  },
        ...
        <MockCard color={c} glow style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
            {["Photography", "Writing", "Music Prod", "Travel", "Cooking", "Side Projects"].map(t => (
              <span key={t} style={{ ...font(11, 500, c), padding: "6px 14px", borderRadius: 100, background: `${c}12`, border: `1px solid ${c}25` }}>{t}</span>
            ))}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            {["Chill Mornings", "Weekend Warrior", "Night Owl Creative"].map(t => (
              <span key={t} style={{ ...font(11, 400, G.textMuted), padding: "6px 14px", borderRadius: 100, background: G.glass, border: `1px solid ${G.glassBorder}` }}>{t}</span>
            ))}
          </div>
        </MockCard>
      </div>
  ...
  },
  {
    render: (c) => (
      <div>
        ...
        <div style={{ ...font(12, 400, G.textDim), fontStyle: "italic", textAlign: "center", padding: "8px 0" }}>
          Inbam senses creative energy dipping &rarr; tells Aram to suggest a recovery day
        </div>
      </div>
    ),
  },
...
const AramDemo = () => <DemoShell color={G.aram} steps={aramSteps} moduleName="Aram" />;
const PorulDemo = () => <DemoShell color={G.porul} steps={porulSteps} moduleName="Porul" />;
const InbamDemo = () => <DemoShell color={G.inbam} steps={inbamSteps} moduleName="Inbam" />;
